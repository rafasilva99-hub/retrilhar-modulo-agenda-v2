...
export function ProdutosPage() {
  ...
  const [tipoCobranca, setTipoCobranca] = useState<
    "variavel" | "fixo" | "exclusivo_item" | "exclusivo_carrinho"
  >("variavel");
  ...
  const [diaPersonalizadoDias, setDiaPersonalizadoDias] = useState<boolean[]>([
    false,
    false,
    false,
    false,
  ...
  const [estoqueQtd, setEstoqueQtd] = useState<Record<string, string>>({
    produto: "40",
    horario: "20",
    evento: "0",
    tarifario: "0",
  ...
  const [saidas, setSaidas] = useState<
    { horario: string; vagas: string; qtdMinima: string; equipe: string }[]
  >([{ horario: "", vagas: "", qtdMinima: "", equipe: "" }]);
  ...
  const [communicationChannels, setCommunicationChannels] = useState<Set<CommunicationChannel>>(
    new Set(["E-mail", "WhatsApp"])
  );
  const [preEventReminderHours, setPreEventReminderHours] = useState("24");
  const [sameDayReminderHours, setSameDayReminderHours] = useState("2");
  const [sendPostEventMessage, setSendPostEventMessage] = useState(true);
  const [voucherInstructions, setVoucherInstructions] = useState(
    "Apresente o voucher no ponto de encontro e leve documento com foto."
  );
  const [preEventMessage, setPreEventMessage] = useState(
    "Sua experiência está chegando. Confira horário, ponto de encontro e itens recomendados antes da saída."
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (statusFilter !== "todos")
      result = result.filter((produto) => produto.status === statusFilter);
    if (search.trim()) {
  ...
  const ticketMedio = activeProducts.length
    ? Math.round(
        activeProducts.reduce((total, produto) => total + produto.preco, 0) / activeProducts.length
      )
    : 0;

  const editingProduto = editingId
    ? produtos.find((produto) => produto.id === editingId)
    : undefined;
  ...
  const openNewProduct = () => {
    ...
    setOpcionais([]);
    setTextoCurto("");
    setCommunicationChannels(new Set(["E-mail", "WhatsApp"]));
    setPreEventReminderHours("24");
    setSameDayReminderHours("2");
    setSendPostEventMessage(true);
    setVoucherInstructions("Apresente o voucher no ponto de encontro e leve documento com foto.");
    setPreEventMessage(
      "Sua experiência está chegando. Confira horário, ponto de encontro e itens recomendados antes da saída."
    );
  };
  ...
  const openEditProduct = (produto: Produto) => {
    ...
    setCommunicationChannels(new Set(["E-mail", "WhatsApp"]));
    setPreEventReminderHours(produto.tipo === "Mercadorias" ? "0" : "24");
    setSameDayReminderHours(produto.tipo === "Mercadorias" ? "0" : "2");
    setSendPostEventMessage(produto.status === "Ativo");
    setVoucherInstructions(
      produto.tipo === "Mercadorias"
        ? "Informe prazo de retirada, conferência do pedido e contato da operação."
        : "Apresente o voucher no ponto de encontro e leve documento com foto."
    );
    setPreEventMessage(
      `${produto.nome}: confira horário, ponto de encontro e orientações antes da experiência.`
    );
  };
  ...
  const archiveProduto = (produto: Produto) => {
    setProdutos((current) => {
      ...
      return current.map((item) =>
        item.id === produto.id ? { ...item, status: "Arquivado" } : item
      );
    });
  ...
  const toggleAllFiltered = () => {
    if (allFilteredSelected) {
      setSelectedIds((current) =>
        current.filter((id) => !filtered.some((produto) => produto.id === id))
      );
      return;
    }
    setSelectedIds((current) =>
  ...
  const tipoGroups: { title: string; tipos: string[] }[] = [
    {
      title: "Experiências com tempo/vaga",
      tipos: [
        "Atividade",
        "Treinamento / aulas",
        "Excursao de 1 dia",
    ...
    {
      title: "Produtos e recorrência",
      tipos: ["Mercadorias", "Aluguel", "Comida e Bebida", "Assinatura", "Cartao presente"],
    },
  ...
  const sidebarGroups = [
      ...
      items: [
        {
          ...
          icon: [
            "M14.5 17.5V7.5H7.50004C5.14304 7.5 3.96454 7.5 3.23231 8.23222C2.50007 8.96445 2.50006 10.1429 2.50004 12.5L2.5 16.5C2.49998 18.857 2.49997 20.0355 3.2322 20.7678C3.96443 21.5 5.14296 21.5 7.5 21.5H10.5C12.3856 21.5 13.3284 21.5 13.9142 20.9142C14.5 20.3284 14.5 19.3856 14.5 17.5Z",
            "M14.4999 16.5H16.4999C18.8569 16.5 20.0354 16.5 20.7676 15.7678C21.4999 15.0355 21.4999 13.857 21.4999 11.5V7.5C21.4999 5.14298 21.4999 3.96447 20.7676 3.23223C20.0354 2.5 18.8569 2.5 16.4999 2.5H9.5L9.50014 7.5",
            "M5.5 12.5H9M5.5 16.5H11.5",
            "M9.5 2.5L14.5 7.5",
          ],
        },
        {
          id: "recorrencia",
          label: "Recorrência e horários",
          icon: [
            ...
            "M22 12C22 6.47715 17.5228 2 12 2",
          ],
        },
        {
          id: "estoque",
          label: "Estoque e vagas",
          icon: [
      ...
      items: [
        {
          ...
          icon: [
            "M2.01758 7.00221C4.21715 7.00221 6.00025 5.2191 6.00025 3.01953",
            "M18 3.01953V3.11144C18 5.26025 19.742 7.00221 21.8908 7.00221",
    ...
    },
    {
      ...
      items: [
        {
          id: "exigencias",
          label: "Exigências de participação",
          icon: [
            "M7 7H15M7 12H12.5M7 17L11 17",
        ...
        },
        {
          ...
          label: "Comunicação",
          icon: [
            "M7.5 12H13.5M7.5 8H10.5",
            "M8.5 20C9.55038 20.8697 10.8145 21.4238 12.2635 21.5188C13.4052 21.5937 14.5971 21.5936 15.7365 21.5188C16.1288 21.4931 16.5565 21.4007 16.9248 21.251C17.3345 21.0845 17.5395 21.0012 17.6437 21.0138C17.7478 21.0264 17.8989 21.1364 18.2011 21.3563C18.7339 21.744 19.4051 22.0225 20.4005 21.9986C20.9038 21.9865 21.1555 21.9804 21.2681 21.7909C21.3808 21.6013 21.2405 21.3389 20.9598 20.8141C20.5706 20.0862 20.324 19.2529 20.6977 18.5852C21.3413 17.6315 21.8879 16.5021 21.9678 15.2823C22.0107 14.6269 22.0107 13.9481 21.9678 13.2927C21.9146 12.4799 21.7173 11.7073 21.4012 11",
            "M12.345 17.4868C15.9006 17.2526 18.7328 14.4069 18.9658 10.8344C19.0114 10.1353 19.0114 9.41131 18.9658 8.71219C18.7328 5.13968 15.9006 2.29401 12.345 2.05985C11.132 1.97997 9.86553 1.98013 8.65499 2.05985C5.09943 2.29401 2.26725 5.13968 2.0342 8.71219C1.9886 9.41131 1.9886 10.1353 2.0342 10.8344C2.11908 12.1356 2.69992 13.3403 3.38372 14.3576C3.78076 15.0697 3.51873 15.9586 3.10518 16.735C2.807 17.2948 2.65791 17.5747 2.77762 17.7769C2.89732 17.9791 3.16472 17.9856 3.69951 17.9985C4.75712 18.024 5.47028 17.7269 6.03638 17.3134C6.35744 17.0788 6.51798 16.9615 6.62862 16.9481C6.73926 16.9346 6.957 17.0234 7.39241 17.2011C7.78374 17.3608 8.23812 17.4593 8.65499 17.4868C9.86553 17.5665 11.132 17.5666 12.345 17.4868Z",
          ],
        },
      ],
    },
  ...
  if (mode !== "list") {
    ...
    const renderPlaceholderSection = (title: string, subtitle: string) => (
      <div>
        <h2 className="font-['Helvetica_Neue:Medium',sans-serif] text-[18px] text-[#181d27]">
          {title}
        </h2>
        <p className="mt-[4px] font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#717680]">
          {subtitle}
        </p>
        <div className="mt-[16px] rounded-xl border border-[#e9eaeb] bg-white p-[24px] shadow-sm">
          <div className="flex flex-col items-center justify-center gap-[8px] py-[24px]">
            ...
            <p className="font-['Helvetica_Neue:Medium',sans-serif] text-[14px] text-[#535862]">
              Em desenvolvimento
            </p>
            <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#a4a7ae]">
      ...
      switch (activeSection) {
          ...
          return (
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_380px]">
                  ...
                  <div className="flex flex-col gap-4 px-4 pt-3 pb-4">
                    <div className="flex flex-col gap-[6px]">
                      <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#181d27]">
                        Nome do produto<span className="text-[#d92d20]"> *</span>
                      </p>
                      <input
                        className={fieldClass}
                        value={form.nome}
                        onChange={(e) => updateForm("nome", e.target.value)}
                        placeholder="Ex: Terra Ronca 1 + Cachoeira da Palmeira"
                      />
                    </div>
                    <div className="flex flex-col gap-[6px]">
                      <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#181d27]">
                    ...
                    <div className="flex flex-col gap-[6px]">
                      ...
                      <textarea
                        ...
                        placeholder="Descritivo e roteiro do passeio"
                      />
                    </div>
                    <div className="flex flex-col gap-[6px]">
                      <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#181d27]">
                      ...
                      <div className="flex cursor-pointer flex-col items-center justify-center gap-[8px] rounded-[8px] border-2 border-dashed border-[#d5d7da] bg-[#fbfcfd] px-[24px] py-[32px] transition-colors hover:border-[#0b5ed7]/40 hover:bg-[#f0f5ff]/30">
                        <svg className="size-[28px] text-[#a4a7ae]" fill="none" viewBox="0 0 24 24">
                          <path
                            d="M12 16V2m0 0l4 4m-4-4L8 6"
                            stroke="currentColor"
                            strokeWidth="1.5"
                            strokeLinecap="round"
              ...
              <div className="flex flex-col gap-[16px]">
                ...
                <div className="rounded-xl border border-[#e9eaeb] bg-white px-4 pt-4 pb-4 shadow-sm">
                  <p className="mb-[10px] font-['Helvetica_Neue:Regular',sans-serif] text-[16px] text-[#181d27]">
                    Tipo de produto
                  </p>
                  <div className="relative flex flex-col gap-[6px]">
                    ...
                    >
                      <span className={form.tipo ? "text-[#252b37]" : "text-[#a4a7ae]"}>
                        {form.tipo ? (tipoLabels[form.tipo] ?? form.tipo) : "Selecione..."}
                      </span>
                      <svg
                        className={`size-[14px] text-[#717680] transition-transform ${tipoDropdownOpen ? "rotate-180" : ""}`}
                        fill="none"
                        viewBox="0 0 24 24"
                      >
                        <path
                    ...
                    {form.tipo && tipoDescriptions[form.tipo] && (
                      <p className="mt-[4px] font-['Helvetica_Neue:Regular',sans-serif] text-[12px] text-[#717680]">
                        {tipoDescriptions[form.tipo]}
                      </p>
                    )}
                    {tipoDropdownOpen && (
                      <>
                        <div
                          ...
                          onClick={() => setTipoDropdownOpen(false)}
                        />
                        <div className="absolute top-[44px] right-0 left-0 z-[61] max-h-[360px] overflow-y-auto rounded-[10px] border border-[#e9eaeb] bg-white shadow-[0_8px_24px_rgba(15,23,43,0.12)]">
                          {tipoGroups.map((group) => (
                            <div key={group.title}>
                              <p className="sticky top-0 bg-[#fafafa] px-[12px] py-[8px] font-['Helvetica_Neue:Medium',sans-serif] text-[11px] tracking-[0.5px] text-[#717680] uppercase">
                                {group.title}
                              </p>
                              {group.tipos.map((tipo) => (
                                ...
                                >
                                  <span
                                    className={`font-['Helvetica_Neue:Regular',sans-serif] text-[13px] ${form.tipo === tipo ? "text-[#0b5ed7]" : "text-[#252b37]"}`}
                                  >
                                    {tipoLabels[tipo] ?? tipo}
                                  </span>
                                  <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[11px] text-[#a4a7ae]">
                ...
                <div className="overflow-visible rounded-xl border border-[#e9eaeb] bg-white px-4 pt-4 pb-4 shadow-sm">
                  <p className="mb-[10px] font-['Helvetica_Neue:Regular',sans-serif] text-[16px] text-[#181d27]">
                    Tags / categorias
                  </p>
                  <div className="flex flex-col gap-[12px]">
                    <div className="relative">
                      <div
                        className={`flex h-[40px] w-full items-center rounded-[8px] border bg-[#fbfcfd] px-[12px] transition-colors ${tagDropdownOpen ? "border-[#0b5ed7] bg-white" : "border-[#e9eaeb]"}`}
                      >
                        <input
                          type="text"
                          className="min-w-0 flex-1 bg-transparent font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#252b37] outline-none placeholder:text-[#a4a7ae]"
                          placeholder={
                            selectedTags.length > 0
                              ? "Buscar ou adicionar tags"
                        ...
                        >
                          <HugeiconsIcon
                            ...
                            size={16}
                            className={`text-[#a4a7ae] transition-transform ${tagDropdownOpen ? "rotate-180" : ""}`}
                          />
                        </button>
                      </div>
                      {tagDropdownOpen &&
                        (() => {
                          ...
                          const allTags = [
                            ...predefinedTags,
                            ...customTags.filter((t) => !predefinedTags.includes(t)),
                          ];
                                ...
                                {filtered.length > 0 ? (
                                  filtered.map((tag) => (
                                    <button
                                      ...
                                      className={`flex w-full cursor-pointer items-center justify-between px-[12px] py-[8px] text-left font-['Helvetica_Neue:Regular',sans-serif] text-[13px] transition-colors hover:bg-[#f8fafc] ${selectedTags.includes(tag) ? "bg-[#f0f5ff] text-[#0b5ed7]" : "text-[#252b37]"}`}
                                    >
                                      <span>{tag}</span>
                                  ...
                                  <div className="py-[4px]">
                                    ...
                                    {trimmed && (
                                      <button
                                        ...
                                        className="flex w-full cursor-pointer items-center gap-[8px] px-[12px] py-[8px] font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#0b5ed7] transition-colors hover:bg-[#f8fafc]"
                                      >
                                        <svg
                                          className="size-[14px] shrink-0"
                                          viewBox="0 0 24 24"
                ...
                <div className="overflow-visible rounded-xl border border-[#e9eaeb] bg-white px-4 pt-4 pb-4 shadow-sm">
                  ...
                  <div className="flex flex-col gap-[12px]">
                    ...
                    <div className="relative">
                      <div
                        className={`flex h-[40px] w-full items-center gap-[8px] rounded-[8px] border px-[12px] transition-colors ${locaisDropdownOpen ? "border-[#0b5ed7] shadow-[0_0_0_1px_#0b5ed7]" : "border-[#e9eaeb] hover:border-[#d0d5dd]"} bg-white`}
                      >
                        <svg
                        ...
                        <span
                          className="flex-1 cursor-pointer font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#a4a7ae]"
                          onClick={() => setLocaisDropdownOpen(!locaisDropdownOpen)}
                        >
                          {locaisAtivos.size > 0
                            ? `${locaisAtivos.size} canal(is) selecionado(s)`
                        ...
                        >
                          <svg
                            className={`size-[14px] text-[#a4a7ae] transition-transform ${locaisDropdownOpen ? "rotate-180" : ""}`}
                            fill="none"
                            viewBox="0 0 24 24"
                          >
                            <path
                      ...
                      {locaisDropdownOpen && (
                        <>
                          <div
                            ...
                            onClick={() => setLocaisDropdownOpen(false)}
                          />
                          <div className="absolute top-full right-0 left-0 z-50 mt-[4px] rounded-[8px] border border-[#e9eaeb] bg-white py-[4px] shadow-[0px_4px_16px_0px_rgba(0,0,0,0.12)]">
                            {["Loja online", "Marketplace", "Balcão", "App mobile", "WhatsApp"].map(
                              (local) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`flex w-full cursor-pointer items-center justify-between px-[12px] py-[8px] text-left font-['Helvetica_Neue:Regular',sans-serif] text-[13px] transition-colors hover:bg-[#f8fafc] ${isSelected ? "bg-[#f0f5ff] text-[#0b5ed7]" : "text-[#252b37]"}`}
                                  >
                                    <span>{local}</span>
                                    {isSelected && (
                                      <svg
                    ...
                    <div className="flex items-center justify-between border-t border-[#f5f5f5] pt-[12px]">
                      <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#535862]">
                        Destacar produto na loja
                      </span>
                      <Switch
                        checked={form.destaque}
                        onCheckedChange={(v) => updateForm("destaque", v)}
                      />
                    </div>
                  </div>
                </div>
          ...
          if (!showDisponibilidade) {
            return (
              <div>
                ...
                <p className="mt-[4px] font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#717680]">
                  Configure quando o produto estará disponível.
                </p>
                <div className="mt-[16px] rounded-xl border border-[#e9eaeb] bg-white p-[24px] shadow-sm">
                  <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#717680]">
                    Não se aplica para o tipo "{form.tipo}".
                  </p>
                </div>
              </div>
            );
          ...
          return (
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_380px]">
              ...
              <div className="flex flex-col gap-6">
                ...
                <section className="rounded-xl border border-[#e9eaeb] bg-white shadow-sm">
                  <div className="px-4 pt-4 pb-0">
                    <h2 className="font-['Helvetica_Neue:Regular',sans-serif] text-[16px] text-[#181d27]">
                      Recorrência
                    </h2>
                  </div>
                  <div className="flex flex-col gap-4 px-4 pt-3 pb-4">
                      ...
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-muted-foreground text-xs">
                            Configure uma programação recorrente para não precisar recriar a
                            atividade toda vez.
                          </span>
                        </div>
                      ...
                      {recorrenciaAtiva && (
                        <div className="space-y-[16px] rounded-[10px] border border-[#f5f5f5] bg-[#fafafa] p-[16px]">
                          ...
                          <div className="flex gap-[4px] rounded-[8px] border border-[#e9eaeb] bg-white p-[4px]">
                            {(["diario", "semanal", "mensal", "personalizado"] as const).map(
                              (tipo) => (
                                <button
                                  key={tipo}
                                  type="button"
                                  onClick={() => setRecorrenciaTipo(tipo)}
                                  className={`flex-1 cursor-pointer rounded-[6px] px-[12px] py-[6px] text-[13px] transition-all ${recorrenciaTipo === tipo ? "bg-[#0b5ed7] font-['Helvetica_Neue:Medium',sans-serif] text-white shadow-sm" : "font-['Helvetica_Neue:Regular',sans-serif] text-[#535862] hover:bg-[#f8fafc] hover:text-[#252b37]"}`}
                                >
                                  {tipo === "diario"
                                    ? "Diário"
                                    : tipo === "semanal"
                          ...
                          <div className="flex items-center gap-[8px]">
                            ...
                            <input
                              className="border-border h-9 w-[56px] rounded-md border bg-white px-3 py-1 text-center text-sm transition-colors outline-none focus:border-[#0b5ed7] focus:ring-3 focus:ring-[#0b5ed7]/30"
                              inputMode="numeric"
                              value={recorrenciaIntervalo}
                              onChange={(e) =>
                                setRecorrenciaIntervalo(e.target.value.replace(/\D/g, ""))
                              }
                            />
                            <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#414651]">
                              {recorrenciaTipo === "diario"
                                  ...
                                  (dia, idx) => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        type="button"
                                        onClick={() => {
                                          const next = [...diasSemana];
                                          next[realIdx] = !next[realIdx];
                                          setDiasSemana(next);
                                        }}
                                        className={`flex h-[36px] min-w-[44px] cursor-pointer items-center justify-center rounded-[8px] border px-[10px] font-['Helvetica_Neue:Regular',sans-serif] text-[13px] transition-all ${diasSemana[realIdx] ? "border-[#0b5ed7] bg-[#0b5ed7] text-white" : "border-[#e9eaeb] bg-white text-[#535862] hover:bg-[#f8fafc]"}`}
                                      >
                                        {dia}
                                      </button>
                                    );
                        ...
                        <div className="flex flex-col gap-[10px]">
                          {saidas.map((saida, idx) => {
                            ...
                            return (
                              <div
                                key={idx}
                                className="border-border bg-muted/40 flex items-center gap-3 rounded-lg border p-3 transition-[opacity,background-color,border-color]"
                              >
                                <button
                                  type="button"
                                  className={`flex size-[20px] shrink-0 items-center justify-center transition-colors ${saidas.length > 1 ? "text-muted-foreground cursor-grab active:cursor-grabbing" : "text-muted-foreground/30"}`}
                                ...
                                <div className="flex shrink-0 items-center gap-2.5">
                                  <button
                                    type="button"
                                    onClick={() => setSaidaDrawerIdx(idx)}
                                    className="border-border text-primary hover:bg-primary/10 flex size-8 cursor-pointer items-center justify-center rounded-md border transition-colors"
                                  >
                                    <svg
                                      className="size-[18px]"
                                      fill="none"
                                  ...
                                  {saidas.length > 1 && (
                                    <button
                                      type="button"
                                      onClick={() =>
                                        setSaidas((prev) => prev.filter((_, i) => i !== idx))
                                      }
                                      className="border-border text-destructive hover:bg-destructive/10 flex size-8 cursor-pointer items-center justify-center rounded-md border transition-colors"
                                    >
                                      <svg
                                        className="size-[18px]"
                                        viewBox="0 0 24 24"
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setSaidaDrawerIdx(saidas.length);
                            }}
                            className="border-border text-primary hover:bg-muted/50 flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed py-2.5 text-sm font-medium transition-colors"
                          >
                            <svg
                              className="size-[16px]"
                              viewBox="0 0 24 24"
                  ...
                  <div className="flex flex-col gap-[12px]">
                    <div className="flex flex-col gap-[6px]">
                      ...
                      <div className="flex gap-[8px]">
                        <input
                          className="h-[40px] min-w-0 flex-1 rounded-[8px] border border-[#e9eaeb] bg-[#fbfcfd] px-[12px] font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#252b37] transition-colors outline-none placeholder:text-[#a4a7ae] focus:border-[#0b5ed7] focus:bg-white"
                          inputMode="numeric"
                          value={duracaoValor}
                          onChange={(e) => setDuracaoValor(e.target.value.replace(/\D/g, ""))}
                          placeholder="4"
                        />
                        <select
                          className="h-[40px] min-w-0 flex-1 rounded-[8px] border border-[#e9eaeb] bg-[#fbfcfd] px-[12px] font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#252b37] transition-colors outline-none focus:border-[#0b5ed7] focus:bg-white"
                    ...
                    <div className="flex flex-col gap-[6px]">
                      <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#535862]">
                        Antecedência mín. de venda
                      </p>
                      <input
                        className={fieldClass}
                        inputMode="numeric"
                        value={antecedenciaVenda}
                        onChange={(e) => setAntecedenciaVenda(e.target.value.replace(/\D/g, ""))}
                        placeholder="24"
                      />
                      <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[11px] text-[#717680]">
                        Até quantas horas antes da saída o produto pode ser vendido.
                      </p>
                    </div>
                  </div>
                </div>
        ...
        case "estoque":
          ...
          return (
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_380px]">
              ...
              <div className="flex flex-col gap-6">
                <section className="rounded-xl border border-[#e9eaeb] bg-white shadow-sm">
                  ...
                  <div className="flex flex-col gap-4 px-4 pt-3 pb-4">
                    ...
                    <div className="flex flex-col gap-[6px]">
                      <p className="font-['Helvetica_Neue:Medium',sans-serif] text-[14px] text-[#181d27]">
                        Onde o estoque é controlado
                      </p>
                      <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] text-[#717680]">
                        Selecione em quais níveis a quantidade é limitada. Os campos de quantidade
                        abaixo acompanham a seleção.
                      </p>
                    </div>
                    <div className="flex gap-[4px] rounded-[8px] border border-[#e9eaeb] bg-white p-[4px]">
                      {(
                        ...
                        return (
                          <button
                            ...
                            className={`flex-1 cursor-pointer rounded-[6px] px-[12px] py-[6px] text-[13px] transition-all ${isActive ? "bg-[#0b5ed7] font-['Helvetica_Neue:Medium',sans-serif] text-white shadow-sm" : "font-['Helvetica_Neue:Regular',sans-serif] text-[#535862] hover:bg-[#f8fafc] hover:text-[#252b37]"}`}
                          >
                            {opt.label}
                          </button>
                        );
                        ...
                        {niveisEstoque.has("produto") && (
                          <div className="flex flex-col gap-[6px]">
                            ...
                            <input
                              className={fieldClass}
                              inputMode="numeric"
                              value={estoqueQtd.produto}
                              onChange={(e) =>
                                setEstoqueQtd((prev) => ({
                                  ...prev,
                                  produto: e.target.value.replace(/\D/g, ""),
                                }))
                              }
                              placeholder="0"
                            />
                        ...
                        {niveisEstoque.has("horario") && (
                          <div className="flex flex-col gap-[6px]">
                            ...
                            <input
                              className={fieldClass}
                              inputMode="numeric"
                              value={estoqueQtd.horario}
                              onChange={(e) =>
                                setEstoqueQtd((prev) => ({
                                  ...prev,
                                  horario: e.target.value.replace(/\D/g, ""),
                                }))
                              }
                              placeholder="0"
                            />
                        ...
                        {niveisEstoque.has("evento") && (
                          <div className="flex flex-col gap-[6px]">
                            ...
                            <input
                              className={fieldClass}
                              inputMode="numeric"
                              value={estoqueQtd.evento}
                              onChange={(e) =>
                                setEstoqueQtd((prev) => ({
                                  ...prev,
                                  evento: e.target.value.replace(/\D/g, ""),
                                }))
                              }
                              placeholder="0"
                            />
                        ...
                        {niveisEstoque.has("tarifario") && (
                          <div className="flex flex-col gap-[6px]">
                            ...
                            <input
                              className={fieldClass}
                              inputMode="numeric"
                              value={estoqueQtd.tarifario}
                              onChange={(e) =>
                                setEstoqueQtd((prev) => ({
                                  ...prev,
                                  tarifario: e.target.value.replace(/\D/g, ""),
                                }))
                              }
                              placeholder="0"
                            />
                ...
                <section className="rounded-xl border border-[#e9eaeb] bg-white shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-4 px-4 pt-3 pb-4">
                    <div className="flex flex-col gap-[6px]">
                      <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#181d27]">
              ...
              <div className="flex flex-col gap-[16px]">
                <div className="rounded-xl border border-[#e9eaeb] bg-white px-4 pt-4 pb-4 shadow-sm">
                  <p className="mb-[12px] font-['Helvetica_Neue:Regular',sans-serif] text-[16px] text-[#181d27]">
                    Controle de estoque e capacidade
                  </p>
                  <div className="flex flex-col divide-y divide-[#f0f1f3]">
                    <div className="flex items-center justify-between gap-[12px] py-[12px] first:pt-0">
                      <div className="flex flex-col gap-[2px]">
                        <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#181d27]">
                          Integração entre empresas
                        </span>
                        <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] text-[#717680]">
                          Compartilha o mesmo estoque com empresas parceiras
                        </span>
                      </div>
                      <Switch />
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-[12px] py-[12px]">
                      <div className="flex flex-col gap-[2px]">
                        ...
                        <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] text-[#717680]">
                          Pode limitar a quantidade disponível por cupom
                        </span>
                      </div>
                      <Switch />
                    </div>
                    <div className="flex items-center justify-between gap-[12px] py-[12px] last:pb-0">
                      ...
                      <Switch
                        checked={permitirOverbooking}
                        onCheckedChange={setPermitirOverbooking}
                      />
                    </div>
                  </div>
                </div>
                ...
                <section className="rounded-xl border border-[#e9eaeb] bg-white shadow-sm">
                  ...
                  <div className="flex flex-col gap-4 px-4 pt-3 pb-4">
                    ...
                    <div className="flex flex-col gap-[6px]">
                      ...
                      <div className="relative">
                        <span className="absolute top-1/2 left-[12px] -translate-y-1/2 font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#717680]">
                          R$
                        </span>
                        <input
                          className={`${fieldClass} pl-[36px]`}
                          inputMode="numeric"
                          value={form.preco}
                          onChange={(e) => {
                      ...
                      <div className="mt-[4px] flex items-center gap-[10px] rounded-[10px] border border-[#f5f5f5] bg-[#f8f9fc] px-[12px] py-[8px]">
                        ...
                        <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] leading-[14px] text-[#414651]">
                          Com vários eventos de valores diferentes, o "a partir de" usa sempre o{" "}
                          <strong className="font-['Helvetica_Neue:Medium',sans-serif]">
                            menor
                          </strong>{" "}
                          valor.
                        </p>
                      ...
                      <div className="flex gap-[4px] rounded-[8px] border border-[#e9eaeb] bg-white p-[4px]">
                        {(
                          ...
                          <button
                            key={opt.value}
                            type="button"
                            onClick={() => setTipoCobranca(opt.value)}
                            className={`flex-1 cursor-pointer rounded-[6px] px-[12px] py-[6px] text-[13px] transition-all ${tipoCobranca === opt.value ? "bg-[#0b5ed7] font-['Helvetica_Neue:Medium',sans-serif] text-white shadow-sm" : "font-['Helvetica_Neue:Regular',sans-serif] text-[#535862] hover:bg-[#f8fafc] hover:text-[#252b37]"}`}
                          >
                            {opt.label}
                          </button>
                        ))}
                          ...
                          <div className="flex gap-[4px] rounded-[8px] border border-[#e9eaeb] bg-white p-[4px]">
                            {(
                              ...
                              <button
                                key={opt.value}
                                type="button"
                                onClick={() => setModeloTarifario(opt.value)}
                                className={`flex-1 cursor-pointer rounded-[6px] px-[12px] py-[6px] text-[13px] transition-all ${modeloTarifario === opt.value ? "bg-[#0b5ed7] font-['Helvetica_Neue:Medium',sans-serif] text-white shadow-sm" : "font-['Helvetica_Neue:Regular',sans-serif] text-[#535862] hover:bg-[#f8fafc] hover:text-[#252b37]"}`}
                              >
                                {opt.label}
                              </button>
                            ))}
                            ...
                            <div className="flex flex-col gap-[10px]">
                              {tarifas.map((tarifa, idx) => {
                                const subtitle = [
                                  ...
                                  tarifa.idadeIni || tarifa.idadeFim
                                    ? `${tarifa.idadeIni || "0"}–${tarifa.idadeFim || "99"} anos`
                                    : "sem faixa etária",
                                  ...
                                  >
                                    <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                                      <span className="text-sm font-medium">
                                        {tarifa.label || "Tarifa sem rótulo"}
                                      </span>
                                      <span className="text-muted-foreground truncate text-xs">
                                        {subtitle}
                                      </span>
                                      ...
                                      <button
                                        ...
                                        onClick={() => {
                                          ...
                                          setTarifaDraft({
                                            label: tarifa.label,
                                            preco: tarifa.preco,
                                            idadeIni: tarifa.idadeIni,
                                            idadeFim: tarifa.idadeFim,
                                            minQty: tarifa.minQty,
                                            maxQty: tarifa.maxQty,
                                          });
                                          setTarifaDrawerIdx(idx);
                                        }}
                                        className="border-border text-primary hover:bg-primary/10 flex size-8 cursor-pointer items-center justify-center rounded-md border transition-colors"
                                      >
                                        <svg
                            ...
                            <div className="flex items-center gap-[8px]">
                              <div className="relative min-w-0 flex-1">
                                <button
                                  ...
                                  className={`flex h-[40px] w-full cursor-pointer items-center justify-between rounded-[8px] border bg-[#fbfcfd] px-[12px] transition-colors ${diaTipoDropdownOpen ? "border-[#0b5ed7] bg-white" : "border-[#e9eaeb]"}`}
                                >
                                  <span
                                    className={`font-['Helvetica_Neue:Regular',sans-serif] text-[14px] ${diaTipoSelecionado ? "text-[#252b37]" : "text-[#a4a7ae]"}`}
                                  >
                                    {diaTipoSelecionado
                                      ? (
                                          {
                                  ...
                                  <HugeiconsIcon
                                    icon={ArrowDown01Icon}
                                    size={16}
                                    className={`text-[#a4a7ae] transition-transform ${diaTipoDropdownOpen ? "rotate-180" : ""}`}
                                  />
                                </button>
                                {diaTipoDropdownOpen && (
                                  <>
                                    <div
                                      className="fixed inset-0 z-[60]"
                                      onClick={() => setDiaTipoDropdownOpen(false)}
                                    />
                                    <div className="absolute top-[44px] right-0 left-0 z-[61] rounded-[8px] border border-[#e9eaeb] bg-white py-[4px] shadow-[0px_4px_16px_0px_rgba(0,0,0,0.12)]">
                                      {[
                                        ...
                                        const isDisabled =
                                          ["seg-sex", "sab-dom"].includes(opt.value) &&
                                          diaRegras.some((r) => r.tipo === opt.value);
                                        return (
                                          <button
                                            key={opt.value}
                                            type="button"
                                            disabled={isDisabled}
                                            onClick={() => {
                                              setDiaTipoSelecionado(opt.value);
                                              setDiaTipoDropdownOpen(false);
                                            }}
                                            className={`flex w-full px-[12px] py-[8px] text-left font-['Helvetica_Neue:Regular',sans-serif] text-[13px] transition-colors ${isDisabled ? "cursor-not-allowed text-[#d0d5dd]" : diaTipoSelecionado === opt.value ? "cursor-pointer bg-[#f0f5ff] text-[#0b5ed7]" : "cursor-pointer text-[#252b37] hover:bg-[#f8fafc]"}`}
                                          >
                                            {opt.label}
                                          </button>
                                        );
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (
                                    ...
                                    !diaPersonalizadoDias.some(Boolean)
                                  )
                                    return;
                                  const dias = ["DOM", "SEG", "TER", "QUA", "QUI", "SEX", "SÁB"];
                                  const tipoKey =
                                    diaTipoSelecionado === "personalizado"
                                      ? `custom-${diaPersonalizadoDias
                                          .map((v, i) => (v ? dias[i] : ""))
                                          .filter(Boolean)
                                          .join(", ")}`
                                      : diaTipoSelecionado;
                                  setDiaRegras((prev) => [
                                    ...prev,
                                    { tipo: tipoKey, preco: diaPrecoInput },
                                  ]);
                                  setDiaTipoSelecionado("");
                                  setDiaPrecoInput("");
                                  setDiaPersonalizadoDias([
                                    false,
                                    false,
                                    false,
                                    false,
                                ...
                                {["DOM", "SEG", "TER", "QUA", "QUI", "SEX", "SÁB"].map(
                                  (dia, idx) => (
                                    <button
                                      ...
                                      type="button"
                                      onClick={() =>
                                        setDiaPersonalizadoDias((prev) =>
                                          prev.map((v, i) => (i === idx ? !v : v))
                                        )
                                      }
                                      className={`flex h-[36px] cursor-pointer items-center justify-center rounded-[8px] border px-[10px] text-[13px] transition-all ${diaPersonalizadoDias[idx] ? "border-[#0b5ed7] bg-[#f0f5ff] font-['Helvetica_Neue:Medium',sans-serif] text-[#0b5ed7]" : "border-[#e9eaeb] bg-white font-['Helvetica_Neue:Regular',sans-serif] text-[#535862] hover:border-[#d0d5dd]"}`}
                                    >
                                      {dia}
                                    </button>
                                  )
                            ...
                            {diaRegras.length > 0 && (
                              <div className="flex flex-wrap gap-[6px]">
                                {diaRegras.map((regra, idx) => {
                                  ...
                                  const chipLabel = regra.tipo.startsWith("custom-")
                                    ? regra.tipo.replace("custom-", "")
                                    : labelMap[regra.tipo] || regra.tipo;
                                  return (
                                    <span
                                      key={idx}
                                    ...
                                    >
                                      ...
                                      <button
                                        type="button"
                                        onClick={() =>
                                          setDiaRegras((prev) => prev.filter((_, i) => i !== idx))
                                        }
                                        className="cursor-pointer transition-colors hover:text-[#084fb7]"
                                      >
                                        <svg
                      ...
                      <div className="flex flex-col gap-[6px]">
                        ...
                        <div className="relative">
                          ...
                          <input
                            className={`${fieldClass} pl-[36px]`}
                            inputMode="numeric"
                            value={valorUnicoProduto}
                            onChange={(e) => setValorUnicoProduto(e.target.value)}
                            placeholder="0,00"
                          />
                        </div>
                        <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] text-[#717680]">
                          Valor fixo, sem variação por configuração.
                        </p>
                      </div>
                    )}
                      ...
                      <div className="grid grid-cols-2 gap-[12px]">
                        <div className="flex flex-col gap-[6px]">
                          ...
                          <div className="relative">
                            <button
                              ...
                              className={`flex h-[40px] w-full cursor-pointer items-center justify-between rounded-[8px] border bg-[#fbfcfd] px-[12px] transition-colors ${proxyDropdownOpen ? "border-[#0b5ed7] bg-white" : "border-[#e9eaeb]"}`}
                            >
                              <span
                                className={`font-['Helvetica_Neue:Regular',sans-serif] text-[14px] ${proxyPagamento ? "text-[#252b37]" : "text-[#a4a7ae]"}`}
                              >
                              ...
                              <HugeiconsIcon
                                icon={ArrowDown01Icon}
                                size={16}
                                className={`text-[#a4a7ae] transition-transform ${proxyDropdownOpen ? "rotate-180" : ""}`}
                              />
                            </button>
                            {proxyDropdownOpen && (
                              <>
                                <div
                                  className="fixed inset-0 z-[60]"
                                  onClick={() => setProxyDropdownOpen(false)}
                                />
                                <div className="absolute top-[44px] right-0 left-0 z-[61] rounded-[8px] border border-[#e9eaeb] bg-white py-[4px] shadow-[0px_4px_16px_0px_rgba(0,0,0,0.12)]">
                                  {[
                                    "Stripe",
                                    ...
                                    <button
                                      ...
                                      type="button"
                                      onClick={() => {
                                        setProxyPagamento(opt);
                                        setProxyDropdownOpen(false);
                                      }}
                                      className={`flex w-full cursor-pointer px-[12px] py-[8px] text-left font-['Helvetica_Neue:Regular',sans-serif] text-[13px] transition-colors ${proxyPagamento === opt ? "bg-[#f0f5ff] text-[#0b5ed7]" : "text-[#252b37] hover:bg-[#f8fafc]"}`}
                                    >
                                      {opt}
                                    </button>
                                  ))}
                        ...
                        <div className="flex flex-col gap-[6px]">
                          ...
                          <input
                            ...
                            onChange={(e) => setParcelasCartao(e.target.value.replace(/\D/g, ""))}
                            placeholder="Ex: 12"
                          />
                        </div>
                      </div>
                    ...
                    {tipoCobranca === "exclusivo_carrinho" && (
                      <div className="flex items-center gap-[10px] rounded-[10px] border border-[#f5f5f5] bg-[#f8f9fc] px-[12px] py-[8px]">
                        ...
                        <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] leading-[14px] text-[#414651]">
                          Cobrança por carrinho: valor fechado para o grupo, definido na aba
                          Financeiro. O tarifário não se aplica. Para cobrar por participante, use
                          Variável (tarifário).
                        </p>
                      </div>
                    )}
                ...
                <div className="rounded-xl border border-[#e9eaeb] bg-white px-4 pt-4 pb-4 shadow-sm">
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex flex-col gap-[2px]">
                      ...
                      <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] text-[#717680]">
                        Cobrar um valor de entrada no momento da reserva
                      </span>
                    </div>
                    ...
                    <div className="mt-[12px] flex flex-col gap-[12px]">
                        ...
                        <div className="flex gap-[4px] rounded-[8px] border border-[#e9eaeb] bg-white p-[4px]">
                          {(
                            ...
                            <button
                              ...
                              className={`flex-1 cursor-pointer rounded-[6px] px-[12px] py-[6px] text-[13px] transition-all ${entradaTipo === opt.value ? "bg-[#0b5ed7] font-['Helvetica_Neue:Medium',sans-serif] text-white shadow-sm" : "font-['Helvetica_Neue:Regular',sans-serif] text-[#535862] hover:bg-[#f8fafc] hover:text-[#252b37]"}`}
                            >
                              {opt.label}
                            </button>
                          ))}
                      ...
                      <div className="flex flex-col gap-[6px]">
                        ...
                        <input
                          ...
                          placeholder={entradaTipo === "porcentagem" ? "Ex: 30%" : "Ex: 50,00"}
                        />
                      </div>
                      {/* Data limite */}
                      <div className="flex flex-col gap-[6px]">
                        ...
                        <input
                          className={fieldClass}
                          value={entradaDataLimite}
                          onChange={(e) => setEntradaDataLimite(e.target.value)}
          ...
          if (!showOpcionais) {
            return (
              <div>
                ...
                <p className="mt-[4px] font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#717680]">
                  Itens adicionais que o cliente pode incluir na compra.
                </p>
                <div className="mt-[16px] rounded-xl border border-[#e9eaeb] bg-white p-[24px] shadow-sm">
                  <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#717680]">
                    Não se aplica para o tipo "{form.tipo}".
            ...
            <div>
              ...
              <div className="mt-[16px] rounded-xl border border-[#e9eaeb] bg-white shadow-sm">
                <div className="flex flex-col gap-[10px] p-[20px]">
                    ...
                    <div key={idx} className="flex items-center gap-[10px]">
                      <input
                        ...
                        onChange={(e) =>
                          setOpcionais((prev) =>
                            prev.map((o, i) =>
                              i === idx ? { nome: e.target.value, preco: o.preco } : o
                            )
                          )
                        }
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setOpcionais((prev) =>
                              prev.map((o, i) =>
                                i === idx
                                  ? { nome: o.nome, preco: e.target.value.replace(/\D/g, "") }
                                  : o
                              )
                            )
                          }
                      ...
                      <button
                        type="button"
                        onClick={() => setOpcionais((prev) => prev.filter((_, i) => i !== idx))}
                        className="flex size-[32px] cursor-pointer items-center justify-center rounded-[6px] text-[#b42318] transition-colors hover:bg-[#fef3f2]"
                      >
                  ...
                  <button
                    type="button"
                    onClick={() => setOpcionais((prev) => [...prev, { nome: "", preco: "" }])}
                    className="flex h-[36px] cursor-pointer items-center gap-[6px] self-start rounded-[8px] border border-dashed border-[#d5d7da] px-[14px] font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#535862] transition-colors hover:bg-[#f8fafc]"
                  >
          ...
          return (
            <div>
              <div className="rounded-xl border border-[#e9eaeb] bg-white shadow-sm">
                ...
                <div className="flex flex-col gap-[20px] px-4 pt-3 pb-4">
                  {/* Proxy de pagamento */}
                  <div className="flex flex-col gap-[6px]">
                    <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#181d27]">
                      Proxy de pagamento
                    ...
                    <div className="relative">
                      <button
                        ...
                        className={`flex h-[40px] w-full cursor-pointer items-center justify-between rounded-[8px] border bg-[#fbfcfd] px-[12px] transition-colors ${pagProxyDropdownOpen ? "border-[#0b5ed7] bg-white" : "border-[#e9eaeb]"}`}
                      >
                        <span
                          className={`font-['Helvetica_Neue:Regular',sans-serif] text-[14px] ${pagProxyPagamento ? "text-[#252b37]" : "text-[#a4a7ae]"}`}
                        >
                        ...
                        <HugeiconsIcon
                          icon={ArrowDown01Icon}
                          size={16}
                          className={`text-[#a4a7ae] transition-transform ${pagProxyDropdownOpen ? "rotate-180" : ""}`}
                        />
                      </button>
                      {pagProxyDropdownOpen && (
                        <>
                          <div
                            className="fixed inset-0 z-[60]"
                            onClick={() => setPagProxyDropdownOpen(false)}
                          />
                          <div className="absolute top-[44px] right-0 left-0 z-[61] rounded-[8px] border border-[#e9eaeb] bg-white py-[4px] shadow-[0px_4px_16px_0px_rgba(0,0,0,0.12)]">
                            {[
                              "Stripe",
                              ...
                              <button
                                ...
                                type="button"
                                onClick={() => {
                                  setPagProxyPagamento(opt);
                                  setPagProxyDropdownOpen(false);
                                }}
                                className={`flex w-full cursor-pointer px-[12px] py-[8px] text-left font-['Helvetica_Neue:Regular',sans-serif] text-[13px] transition-colors ${pagProxyPagamento === opt ? "bg-[#f0f5ff] text-[#0b5ed7]" : "text-[#252b37] hover:bg-[#f8fafc]"}`}
                              >
                                {opt}
                              </button>
                            ))}
                      ...
                      {(
                        [
                          {
                            ...
                            icon: (
                              <svg
                                ...
                                strokeLinejoin="round"
                              >
                                <path d="M22 10H2M14.5 14.5H17.5" />
                                <path d="M5.75 5H18.25C20.3211 5 22 6.67893 22 8.75V16.25C22 18.3211 20.3211 20 18.25 20H5.75C3.67893 20 2 18.3211 2 16.25V8.75C2 6.67893 3.67893 5 5.75 5Z" />
                              </svg>
                            ),
                            subtitle: "Débito e crédito via gateway",
                          },
                          {
                            id: "Pix",
                            icon: (
                          ...
                          },
                          {
                            ...
                            icon: (
                              ...
                              >
                                ...
                                <path d="M5 4V20" />
                                <path d="M15 4V20" />
                                <path d="M8 18V6C8 5.05719 8 4.58579 8.29289 4.29289C8.58579 4 9.05719 4 10 4C10.9428 4 11.4142 4 11.7071 4.29289C12 4.58579 12 5.05719 12 6V18C12 18.9428 12 19.4142 11.7071 19.7071C11.4142 20 10.9428 20 10 20C9.05719 20 8.58579 20 8.29289 19.7071C8 19.4142 8 18.9428 8 18Z" />
                                <path d="M18 18V6C18 5.05719 18 4.58579 18.2929 4.29289C18.5858 4 19.0572 4 20 4C20.9428 4 21.4142 4 21.7071 4.29289C22 4.58579 22 5.05719 22 6V18C22 18.9428 22 19.4142 21.7071 19.7071C21.4142 20 20.9428 20 20 20C19.0572 20 18.5858 20 18.2929 19.7071C18 19.4142 18 18.9428 18 18Z" />
                              </svg>
                            ),
                          ...
                          },
                          {
                            ...
                            icon: (
                              <svg
                                ...
                                strokeLinejoin="round"
                              >
                                <path d="M2.01758 15C4.21715 15 6.00025 16.7831 6.00025 18.9827" />
                                <path d="M18 18.9827V18.8908C18 16.742 19.742 15 21.8908 15" />
                                <path d="M6.00025 5.01562C6.00025 7.2152 4.21715 8.9983 2.01758 8.9983" />
                                <path d="M18 5.01562C18 7.19594 19.769 8.96705 21.9423 8.99789" />
                                <path d="M16 5H8C5.17157 5 3.75736 5 2.87868 5.87868C2 6.75736 2 8.17157 2 11V13C2 15.8284 2 17.2426 2.87868 18.1213C3.75736 19 5.17157 19 8 19H16C18.8284 19 20.2426 19 21.1213 18.1213C22 17.2426 22 15.8284 22 13V11C22 8.17157 22 6.75736 21.1213 5.87868C20.2426 5 18.8284 5 16 5Z" />
                                <path d="M15 12C15 13.6569 13.6569 15 12 15C10.3431 15 9 13.6569 9 12C9 10.3431 10.3431 9 12 9C13.6569 9 15 10.3431 15 12Z" />
                              </svg>
                            ),
                            subtitle: "Pagamento presencial",
                          },
                        ] as const
                      ).map((forma) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              setPagFormasAceitas((prev) => {
                                const next = new Set(prev);
                                if (next.has(forma.id)) next.delete(forma.id);
                                else next.add(forma.id);
                                return next;
                              })
                            }
                            className={`flex cursor-pointer items-start gap-[10px] rounded-[10px] border px-[14px] py-[12px] text-left transition-all ${isActive ? "border-[#0b5ed7] bg-[#f0f5ff]" : "border-[#e9eaeb] bg-white hover:bg-[#f8fafc]"}`}
                          >
                            <div
                              className={`mt-[1px] shrink-0 ${isActive ? "text-[#0b5ed7]" : "text-[#717680]"}`}
                            >
                              {forma.icon}
                            </div>
                            <div className="flex min-w-0 flex-col gap-[2px]">
                              <span
                                className={`font-['Helvetica_Neue:Medium',sans-serif] text-[13px] ${isActive ? "text-[#0b5ed7]" : "text-[#252b37]"}`}
                              >
                                {forma.id}
                              </span>
                              <span
                                className={`font-['Helvetica_Neue:Regular',sans-serif] text-[11px] ${isActive ? "text-[#0b5ed7]/70" : "text-[#717680]"}`}
                              >
                                {forma.subtitle}
                              </span>
                            </div>
                  ...
                  {pagFormasAceitas.has("Cartão de crédito") && (
                    <div className="flex flex-col gap-[6px]">
                      ...
                      <input
                        ...
                        inputMode="numeric"
                        value={pagParcelas}
                        onChange={(e) => setPagParcelas(e.target.value.replace(/\D/g, ""))}
                        placeholder="12"
                      />
                      <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] text-[#717680]">
        ...
        case "canais":
          return renderPlaceholderSection(
            ...
            "Defina onde o produto será exibido e vendido."
          );
        case "exigencias":
          return renderPlaceholderSection(
            "Exigências de participação",
            "Requisitos que os participantes devem atender."
          );
        case "termos":
          return renderPlaceholderSection(
            "Termos e contratos",
            "Documentos e termos associados ao produto."
          );
        case "comunicacao": {
          const channels: readonly CommunicationChannel[] = ["E-mail", "WhatsApp", "SMS"];
          const selectedChannelsLabel =
            Array.from(communicationChannels).join(", ") || "Nenhum canal ativo";
          const remindersEnabled =
            preEventReminderHours.trim() !== "0" || sameDayReminderHours.trim() !== "0";
          ...
          return (
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_380px]">
                ...
                <section className="rounded-xl border border-[#e9eaeb] bg-white shadow-sm">
                  <div className="px-4 pt-4 pb-0">
                    ...
                    <p className="mt-[4px] font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#717680]">
                      Configure os avisos enviados ao participante após a compra e antes da saída.
                    </p>
                  </div>
                  <div className="flex flex-col gap-4 px-4 pt-3 pb-4">
                    <div className="flex flex-col gap-[8px]">
                      ...
                      <div className="grid grid-cols-1 gap-[8px] md:grid-cols-3">
                        {channels.map((channel) => {
                          const isActive = communicationChannels.has(channel);
                          return (
                            <button
                              key={channel}
                              type="button"
                              ...
                              className={`flex h-[42px] cursor-pointer items-center justify-between rounded-[8px] border px-[12px] transition-colors ${isActive ? "border-[#0b5ed7] bg-[#f0f5ff] text-[#0b5ed7]" : "border-[#e9eaeb] bg-[#fbfcfd] text-[#535862] hover:bg-[#f8fafc]"}`}
                            >
                              <span className="font-['Helvetica_Neue:Medium',sans-serif] text-[13px]">
                                {channel}
                              </span>
                              <span
                                className={`size-[8px] rounded-full ${isActive ? "bg-[#0b5ed7]" : "bg-[#d5d7da]"}`}
                              />
                            </button>
                          );
                        })}
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      <div className="flex flex-col gap-[6px]">
                        ...
                        <div className="flex items-center gap-[8px]">
                          <input
                            className={`${fieldClass} w-[96px] text-center`}
                            inputMode="numeric"
                            value={preEventReminderHours}
                            onChange={(e) =>
                              setPreEventReminderHours(e.target.value.replace(/\D/g, ""))
                            }
                            aria-label="Horas antes para lembrete pré-evento"
                          />
                          <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#535862]">
                            horas antes
                          </span>
                      ...
                      <div className="flex flex-col gap-[6px]">
                        ...
                        <div className="flex items-center gap-[8px]">
                          <input
                            className={`${fieldClass} w-[96px] text-center`}
                            inputMode="numeric"
                            value={sameDayReminderHours}
                            onChange={(e) =>
                              setSameDayReminderHours(e.target.value.replace(/\D/g, ""))
                            }
                            aria-label="Horas antes para reforço no dia"
                          />
                          <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#535862]">
                            horas antes
                          </span>
                    ...
                    <div className="flex items-center justify-between gap-4 rounded-[10px] border border-[#f0f1f3] bg-[#fafafa] px-[14px] py-[12px]">
                      <div className="flex min-w-0 flex-col gap-[2px]">
                        <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#181d27]">
                          Enviar resumo pós-evento
                        </span>
                        <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] text-[#717680]">
                          Solicita avaliação e compartilha próximos passos com o participante.
                        </span>
                      </div>
                      <Switch
                        checked={sendPostEventMessage}
                        onCheckedChange={setSendPostEventMessage}
                      />
                    </div>
                  </div>
                </section>
                ...
                <section className="rounded-xl border border-[#e9eaeb] bg-white shadow-sm">
                  ...
                  <div className="flex flex-col gap-4 px-4 pt-3 pb-4">
                    <div className="flex flex-col gap-[6px]">
                      ...
                      <textarea
                        className={textAreaClass}
                        value={preEventMessage}
                        onChange={(e) => setPreEventMessage(e.target.value)}
                        placeholder="Escreva a mensagem enviada antes da experiência"
                      />
                    </div>
                    <div className="flex flex-col gap-[6px]">
                      <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#181d27]">
                        Instruções no voucher
                      </p>
                      <textarea
                        className={textAreaClass}
                        value={voucherInstructions}
                        onChange={(e) => setVoucherInstructions(e.target.value)}
                        placeholder="Informe documentos, itens recomendados e ponto de encontro"
                      />
                    </div>
                  </div>
                </section>
              ...
              <div className="flex flex-col gap-[16px]">
                <section className="rounded-xl border border-[#e9eaeb] bg-white px-4 pt-4 pb-4 shadow-sm">
                  <h2 className="font-['Helvetica_Neue:Regular',sans-serif] text-[16px] text-[#181d27]">
                    Resumo de envio
                  </h2>
                  <div className="mt-[12px] flex flex-col divide-y divide-[#f0f1f3]">
                    {[
                      { label: "Produto", value: form.nome.trim() || "Produto sem nome" },
                      { label: "Canais", value: selectedChannelsLabel },
                      {
                        label: "Lembretes",
                        value: remindersEnabled ? "Pré-evento configurado" : "Desativados",
                      },
                      {
                        label: "Pós-evento",
                        value: sendPostEventMessage ? "Ativo" : "Inativo",
                      },
                    ].map((item) => (
                      <div key={item.label} className="flex justify-between gap-[12px] py-[10px]">
                        <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#717680]">
                          {item.label}
                        </span>
                        <span className="max-w-[190px] text-right font-['Helvetica_Neue:Medium',sans-serif] text-[13px] text-[#252b37]">
                          {item.value}
                        </span>
                      </div>
                    ))}
                ...
                <section className="rounded-xl border border-[#e9eaeb] bg-[#f8f9fc] px-4 pt-4 pb-4">
                  <h2 className="font-['Helvetica_Neue:Regular',sans-serif] text-[16px] text-[#181d27]">
                    Prévia do voucher
                  </h2>
                  <div className="mt-[12px] rounded-[10px] border border-[#e9eaeb] bg-white p-[14px]">
                    <p className="font-['Helvetica_Neue:Medium',sans-serif] text-[13px] text-[#252b37]">
                      {form.nome.trim() || "Novo produto"}
                    </p>
                    <p className="mt-[8px] font-['Helvetica_Neue:Regular',sans-serif] text-[13px] leading-[18px] text-[#535862]">
                      {voucherInstructions.trim() || "Sem instruções adicionais."}
                    </p>
                    <div className="mt-[12px] rounded-[8px] bg-[#f0f5ff] px-[10px] py-[8px]">
                      <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] leading-[16px] text-[#0b5ed7]">
                        {preEventMessage.trim() || "Mensagem pré-evento não preenchida."}
                      </p>
                    </div>
                  </div>
                </section>
    ...
    return (
      <div className="fixed inset-0 z-50 flex flex-col bg-[#f8fafc]">
        ...
        <div className="flex h-[56px] shrink-0 items-center border-b border-[#e9eaeb] bg-white px-[12px] md:px-[20px]">
          <div className="flex min-w-0 items-center gap-[12px] md:gap-[16px]">
            ...
            />
            <div className="hidden h-[20px] w-px shrink-0 bg-[#e9eaeb] sm:block" />
            <p className="hidden min-w-0 truncate font-['Helvetica_Neue:Regular',sans-serif] text-[14px] text-[#252b37] sm:block">
              {form.nome.trim() || (mode === "new" ? "Novo produto" : "Editar produto")}
            </p>
          </div>
          <div className="ml-auto flex shrink-0 items-center gap-[8px] md:gap-[10px]">
            <button
              type="button"
              onClick={(e) => saveProduto(e as unknown as FormEvent)}
              className="flex cursor-pointer items-center gap-[6px] rounded-[8px] bg-[#0b5ed7] px-[14px] py-[8px] font-['Helvetica_Neue:Medium',sans-serif] text-[13px] whitespace-nowrap text-white transition-colors hover:bg-[#084fb7]"
            >
        ...
        <div className="flex flex-1 flex-col overflow-hidden md:flex-row">
          {/* Sidebar nav */}
          <nav className="flex max-h-[220px] w-full shrink-0 flex-col gap-[20px] overflow-y-auto border-b border-[#e9eaeb] bg-white px-[12px] py-[16px] md:max-h-none md:w-[250px] md:border-r md:border-b-0 md:py-[20px]">
            {sidebarGroups.map((group) => (
              <div key={group.title} className="space-y-[4px]">
                <p className="text-muted-foreground px-[0.75em] pb-[0.25em] text-[10px] font-medium tracking-widest uppercase">
                  {group.title}
                </p>
                {group.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full cursor-pointer items-center gap-[8px] rounded-[8px] px-[12px] py-[8px] text-left text-[14px] whitespace-nowrap transition-colors ${isActive ? "bg-[#eff6ff] font-['Helvetica_Neue:Regular',sans-serif] text-[#0b5ed7]" : "font-['Helvetica_Neue:Regular',sans-serif] text-[#535862] hover:bg-[#f5f5f5]/50 hover:text-[#252b37]"}`}
                    >
                      <svg
          ...
          {/* Content area */}
          <div className="flex-1 overflow-y-auto px-[16px] py-[16px] md:px-[32px] md:py-[24px]">
        ...
        {saidaDrawerIdx !== null &&
          (() => {
            ...
            if (!saida) {
              setSaidaDrawerIdx(null);
              return null;
            }
            const updateSaida = (key: string, value: string) =>
              setSaidas((prev) =>
                prev.map((s, i) => (i === saidaDrawerIdx ? { ...s, [key]: value } : s))
              );
            return createPortal(
              <div className="fixed inset-0 z-[60] flex justify-end">
                <div
                  className="absolute inset-0 bg-black/40"
                  onClick={() => setSaidaDrawerIdx(null)}
                />
                <div className="animate-in slide-in-from-right relative z-10 flex max-h-full w-[480px] flex-col overflow-hidden rounded-l-[16px] border border-[#e9eaeb] bg-white shadow-[-8px_0px_24px_0px_rgba(0,0,0,0.1)] duration-200">
                  {/* Header */}
                  <div className="shrink-0">
                    <div className="flex items-center justify-between px-6 pt-5 pb-[16px]">
                      <p className="font-['Helvetica_Neue:Medium',sans-serif] text-[16px] text-[#181d27]">
                        Configurar saída {saidaDrawerIdx + 1}
                      </p>
                      <button
                        type="button"
                        onClick={() => setSaidaDrawerIdx(null)}
                  ...
                  {/* Content */}
                  <div className="flex-1 space-y-6 overflow-y-auto px-6 pt-0 pb-5">
                    {/* Horários */}
                    <div>
                      <div
                      ...
                      <div className="grid grid-cols-2 gap-4 pt-5">
                        <div className="flex flex-col gap-1.5">
                          <label className="flex items-center gap-2 text-sm leading-none font-normal">
                            Início
                          </label>
                          <input
                            type="text"
                            ...
                            placeholder="hh:mm"
                            value={saida.horario}
                            onChange={(e) => updateSaida("horario", e.target.value)}
                          />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <label className="flex items-center gap-2 text-sm leading-none font-normal">
                      ...
                      <div className="grid grid-cols-2 gap-4 pt-5">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            ...
                            inputMode="numeric"
                            placeholder="0"
                            value={saida.qtdMinima}
                            onChange={(e) =>
                              updateSaida("qtdMinima", e.target.value.replace(/\D/g, ""))
                            }
                          />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            ...
                            value={saida.vagas}
                            onChange={(e) =>
                              updateSaida("vagas", e.target.value.replace(/\D/g, ""))
                            }
                          />
                        </div>
                      </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="relative">
                        <div className="flex h-[40px] w-full items-center gap-[8px] rounded-[8px] border border-[#e9eaeb] px-[12px] transition-colors hover:border-[#d0d5dd]">
                          ...
                          <input
                            type="text"
                            className="flex-1 bg-transparent font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#252b37] outline-none placeholder:text-[#a4a7ae]"
                            placeholder="Buscar ou adicionar membro..."
                          />
                          <button type="button" className="shrink-0 cursor-pointer">
                            <svg
                              className="size-[14px] text-[#a4a7ae]"
                              fill="none"
                              viewBox="0 0 24 24"
                            >
                              <path
        ...
        {/* Tarifa drawer */}
        {tarifaDrawerIdx !== null &&
          tarifaDraft !== null &&
          (() => {
            const tarifa = tarifaDraft;
            const updateTarifa = (key: string, value: string) =>
              setTarifaDraft((prev) => (prev ? { ...prev, [key]: value } : prev));
            return createPortal(
              <div className="fixed inset-0 z-[60] flex justify-end">
                <div
                  className="absolute inset-0 bg-black/40"
                ...
                <div className="animate-in slide-in-from-right relative z-10 flex max-h-full w-[480px] flex-col overflow-hidden rounded-l-[16px] border border-[#e9eaeb] bg-white shadow-[-8px_0px_24px_0px_rgba(0,0,0,0.1)] duration-200">
                  ...
                  <div className="flex-1 space-y-6 overflow-y-auto px-6 pt-0 pb-5">
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-4 pt-5">
                        <div className="flex flex-col gap-1.5">
                          <label className="text-sm leading-none font-normal">
                            Título da tarifa
                          </label>
                          <input
                            className={fieldClass}
                            value={tarifa.label}
                            onChange={(e) => updateTarifa("label", e.target.value)}
                      ...
                      <div className="grid grid-cols-2 gap-4 pt-5">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            className={fieldClass}
                            inputMode="numeric"
                            value={tarifa.idadeIni}
                            onChange={(e) =>
                              updateTarifa("idadeIni", e.target.value.replace(/\D/g, ""))
                            }
                            placeholder="0"
                          />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            className={fieldClass}
                            inputMode="numeric"
                            value={tarifa.idadeFim}
                            onChange={(e) =>
                              updateTarifa("idadeFim", e.target.value.replace(/\D/g, ""))
                            }
                            placeholder="99"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 pt-5">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            ...
                            value={tarifa.minQty}
                            onChange={(e) =>
                              updateTarifa("minQty", e.target.value.replace(/\D/g, ""))
                            }
                            placeholder="1"
                          />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            ...
                            value={tarifa.maxQty}
                            onChange={(e) =>
                              updateTarifa("maxQty", e.target.value.replace(/\D/g, ""))
                            }
                            placeholder="10"
                          />
                        </div>
                  ...
                  <div className="flex shrink-0 items-center justify-end gap-[12px] border-t border-[#e9eaeb] px-6 py-4">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (tarifaDraft && tarifaDrawerIdx !== null) {
                          setTarifas((prev) =>
                            prev.map((t, i) => (i === tarifaDrawerIdx ? tarifaDraft : t))
                          );
                        }
                        setTarifaDraft(null);
                        setTarifaDrawerIdx(null);
        ...
        {loteDrawerIdx !== null &&
          ...
          (() => {
            ...
            const updateLote = (key: string, value: string) =>
              setLoteDraft((prev) => (prev ? { ...prev, [key]: value } : prev));
            return createPortal(
              <div className="fixed inset-0 z-[60] flex justify-end">
                <div
                  className="absolute inset-0 bg-black/40"
                ...
                <div className="animate-in slide-in-from-right relative z-10 flex max-h-full w-[480px] flex-col overflow-hidden rounded-l-[16px] border border-[#e9eaeb] bg-white shadow-[-8px_0px_24px_0px_rgba(0,0,0,0.1)] duration-200">
                      ...
                      <div className="flex flex-col gap-4 pt-5">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            className={fieldClass}
                            value={lote.label}
                            onChange={(e) => updateLote("label", e.target.value)}
                            placeholder="Ex.: Grupo pequeno"
                          />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <label className="text-sm leading-none font-normal">Preço</label>
                      ...
                      <div className="grid grid-cols-2 gap-4 pt-5">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            ...
                            inputMode="numeric"
                            value={lote.de}
                            onChange={(e) => updateLote("de", e.target.value.replace(/\D/g, ""))}
                            placeholder="1"
                          />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            ...
                            inputMode="numeric"
                            value={lote.ate}
                            onChange={(e) => updateLote("ate", e.target.value.replace(/\D/g, ""))}
                            placeholder="10"
                          />
                        </div>
                      </div>
                  ...
                  <div className="flex shrink-0 items-center justify-end gap-[12px] border-t border-[#e9eaeb] px-6 py-4">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (loteDraft && loteDrawerIdx !== null) {
                          setLotes((prev) =>
                            prev.map((l, i) => (i === loteDrawerIdx ? loteDraft : l))
                          );
                        }
                        setLoteDraft(null);
                        setLoteDrawerIdx(null);
          ...
          {[
            {
              ...
              icon: (
                <svg className="block size-full" fill="none" viewBox="0 0 16 16">
                  <path
                    d="M11.333 0.667V2.667M4.667 0.667V2.667"
                    stroke="#0B5ED7"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M8 12.667H7.333C5.152 12.667 4.062 12.667 3.448 12.052C2.833 11.438 2.833 10.348 2.833 8.167V7.167C2.833 4.986 2.833 3.895 3.448 3.281C4.062 2.667 5.152 2.667 7.333 2.667H8.667C10.848 2.667 11.938 2.667 12.552 3.281C13.167 3.895 13.167 4.986 13.167 7.167V7.833"
                    stroke="#0B5ED7"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M2.833 5.333H13.167"
                    stroke="#0B5ED7"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M10.333 12.667C10.333 12.667 12.333 11.681 12.333 10.083C12.333 9.455 11.886 8.947 11.273 8.947C10.77 8.947 10.435 9.166 10.211 9.603C9.988 9.166 9.653 8.947 9.15 8.947C8.537 8.947 8.09 9.455 8.09 10.083C8.09 11.681 10.333 12.667 10.333 12.667Z"
                    stroke="#0B5ED7"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
              ...
              icon: (
                <svg className="block size-full" fill="none" viewBox="0 0 16 16">
                  <path
                    ...
                    strokeLinejoin="round"
                  />
                  <circle cx="8" cy="8" r="6.5" stroke="#0B5ED7" strokeWidth="1.2" />
                </svg>
              ),
            },
            {
              label: "Inativos",
              subtitle: "Produtos desativados",
            ...
            {
              label: "Ticket médio",
              subtitle: "Valor médio por produto",
              value: formatCurrency(ticketMedio),
              icon: (
                <svg className="block size-full" fill="none" viewBox="0 0 16 16">
                  <path
                    d="M2.333 10.264V5.358C2.333 3.842 2.333 3.084 2.8 2.612C3.267 2.14 4.017 2.14 5.517 2.14H7.633C9.133 2.14 9.883 2.14 10.35 2.612C10.817 3.084 10.817 3.842 10.817 5.358V10.264C10.817 11.068 10.817 11.47 10.572 11.628C10.172 11.886 9.553 11.345 9.241 11.148C8.984 10.986 8.855 10.905 8.712 10.9C8.558 10.895 8.427 10.974 8.149 11.148L7.136 11.789C6.863 11.962 6.726 12.048 6.574 12.048C6.422 12.048 6.285 11.962 6.012 11.789L4.999 11.148C4.742 10.986 4.613 10.905 4.47 10.9C4.316 10.895 4.185 10.974 3.907 11.148C3.596 11.345 2.976 11.886 2.576 11.628C2.333 11.47 2.333 11.068 2.333 10.264Z"
                    stroke="#0B5ED7"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M8.683 4.333H5.183"
                    stroke="#0B5ED7"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
            ...
            <div
              ...
              data-slot="card"
              data-size="default"
              className="group/card bg-card text-card-foreground ring-foreground/5 flex flex-col gap-0 overflow-hidden rounded-4xl py-0 text-sm shadow-none ring-1"
            >
              <div data-slot="card-content" className="h-full p-[1.25em]">
                <div className="flex h-full items-start justify-between gap-[0.75em]">
                  ...
                  <div className="bg-primary/10 flex size-[2.5em] shrink-0 items-center justify-center rounded-lg">
                    <div className="text-primary size-[20px]">{card.icon}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center gap-[0.75em]">
          ...
          <div className="relative hidden md:block">
            <button
              ...
              className="group/button border-border bg-background hover:bg-muted hover:text-foreground inline-flex h-9 shrink-0 items-center justify-center gap-1.5 rounded-lg border px-3 font-['Helvetica_Neue:Regular',sans-serif] text-sm text-[#414651] transition-all"
            >
        ...
        <div>
          <div className="overflow-hidden rounded-[12px] border border-[#e9eaeb] bg-white">
            <div className="flex h-[44px] items-center border-b border-[#e9eaeb] bg-[#fafafa] px-[16px]">
              ...
              <p className="w-[80px] shrink-0 text-center font-['Helvetica_Neue:Medium',sans-serif] text-[12px] tracking-[0.5px] text-[#717680] uppercase">
                Duração
              </p>
              <p className="w-[100px] shrink-0 text-center font-['Helvetica_Neue:Medium',sans-serif] text-[12px] tracking-[0.5px] text-[#717680] uppercase">
                Capacidade
              </p>
              <p className="w-[100px] shrink-0 font-['Helvetica_Neue:Medium',sans-serif] text-[12px] tracking-[0.5px] text-[#717680] uppercase">
                Última venda
              ...
              filtered.map((produto, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="w-[80px] shrink-0 text-center font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#535862]">
                      {produto.duracao}
                    </p>
                    <p className="w-[100px] shrink-0 text-center font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#535862]">
                      {produto.capacidade === null ? "-" : `${produto.capacidade} pax`}
                    </p>
                    <p className="w-[100px] shrink-0 font-['Helvetica_Neue:Regular',sans-serif] text-[13px] text-[#535862]">
                      {produto.ultimaVenda}
                    </p>
            ...
            <div className="flex items-center gap-[8px]">
              ...
              <div className="flex items-center gap-[4px]">
                {["«", "‹", "›", "»"].map((arrow, i) => (
                  ...
                  >
                    <span className="font-['Helvetica_Neue:Regular',sans-serif] text-[14px]">
                      {arrow}
                    </span>
                  </button>
                ))}
