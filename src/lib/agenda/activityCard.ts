...
export function getContextualBadges(activity: Activity): ContextualBadge[] {
  ...
  if (!activity.requiresInsurance) {
    ...
    badges.push({ type: 'insurance', label: 'Todos os participantes estão assegurados', color: 'green', icon: 'shield-check' });
  } else {
    badges.push({ type: 'insurance', label: 'Seguro obrigatório', color: 'red', icon: 'shield-alert' });
  }
...
export function getMultiDayRange(activity: Activity): string | null {
  if (activity.activityType !== 'multi-dias' || !activity.startDate || !activity.endDate || !activity.totalDays) {
    return null;
  }
